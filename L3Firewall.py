...
class Firewall (EventMixin):
	...
	def __init__ (self,l2config,l3config):
		...
		self.seenMACstoIPs = {} # Keep track of the first IP we see for each MAC address
		self.seenPortsstoMACs = {} # Keep track of the first IP we see for each MAC address
		self.macToTimeToCount = {} # Fairly bad attempt to limit 
	...
	def _handle_ConnectionUp (self, event):
		...
		for (source, destination) in self.disbaled_MAC_pair:
			...
			message.match = match			
			self.connection.send(message) # Send instruction to the switch

		log.debug("Firewall rules installed on %s", dpidToStr(event.dpid))

	# Handles minimal port security requirements
	def handleSpoofedIps(self, packet, mac_addr, ip_addr, event):
		  if mac_addr in self.seenMACstoIPs.keys():
			#Allow the initial IP address associated with the MAC until a spoof is found
			if self.seenMACstoIPs[mac_addr] != ip_addr:
				# If the IP has already been used with this MAC add a flow to block it
				msg = of.ofp_flow_mod()
				msg.match = of.ofp_match()
				# Set source and destination macs to drop (no action means drop)
				msg.match.dl_src = mac_addr
				msg.match.dl_dst = packet.dst
				msg.priority = self.priorityCtr
				msg.hard_timeout = 0
				msg.idle_timeout = 200
				self.priorityCtr = self.priorityCtr + 1
				event.connection.send(msg)			
		  else:
			#Record unseen MAC address and link to ip address
			self.seenMACstoIPs[mac_addr] = ip_addr
	
	# Attempting to handle MAC spoofing based on switch port
	def handleSpoofedMACs(self, packet, match, port, event, mac_addr):
		  #mac_addr = packet.src

		  if port in self.seenPortsstoMACs.keys():
			#Allow the initial MAC address associated with the port until a spoof is found
			if self.seenPortsstoMACs[port] != mac_addr:
				# If the MAC has already been used with this port add a flow to block it
				msg = of.ofp_flow_mod()
				msg.match = of.ofp_match()
				# Set source port and destination macs to drop (no action means drop)
			...
			#print "Else"
			#Record unseen MAC address and link to ip address
			self.seenPortsstoMACs[port] = mac_addr

	def handleIPRate(self, mac_addr_src, mac_addr_dst, ip_addr, event):
		# Current time
		curr_second = int(time.time())
		# Have we seen the mac
		if mac_addr_src in self.macToTimeToCount.keys():
			# When was the mac address last seen
			last_second_seen = self.macToTimeToCount[mac_addr_src].keys()[0]
			# Did we see the mac this second
			if curr_second == last_second_seen:

				# How many times
				curr_count = self.macToTimeToCount[mac_addr_src][curr_second]
				# More than our limit?
				if curr_count > 5:
					# Add rule to block mac
					msg = of.ofp_flow_mod()
					msg.match = of.ofp_match()
					# Set source port and destination macs to drop (no action means drop)
					msg.match.dl_src = mac_addr_src
					msg.match.dl_dst = mac_addr_dst
					msg.priority = self.priorityCtr
					self.priorityCtr = self.priorityCtr + 1
					event.connection.send(msg)
				else:
					#Update count of times seen this second
					self.macToTimeToCount[mac_addr_src][curr_second] = curr_count + 1
			else:
				# Reset for new second
				self.macToTimeToCount[mac_addr_src] = {curr_second : 1 }
		else:
			# Add record for mac address
			self.macToTimeToCount[mac_addr_src] = {curr_second : 1 }
	...
	def _handle_PacketIn(self, event):		
		...
		# Get the source MAC of the incoming packet
		mac_addr = packet.src
		...
		if(match.dl_type == packet.IP_TYPE):
		  ...
		  ip_addr = ip_packet.srcip 

		  self.handleSpoofedIps(packet, mac_addr, ip_addr, event)
		  self.handleSpoofedMACs(packet, match, event.port, event, mac_addr)
		  self.handleIPRate(packet.src, packet.dst, ip_addr, event)
		  if ip_packet.protocol == ip_packet.TCP_PROTOCOL:
			log.debug("ThCP it is !")
